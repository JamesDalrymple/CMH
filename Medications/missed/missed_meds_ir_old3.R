...
for_review <- data.table(auth_case_no=numeric(), vendor_short=character(),
                         mm_ir_cases=integer(), num_IRs=integer(), qtr=character(), vendor_name=character())
mm_plots <- NULL
...
tmp_v_names <- unique(ir_detail[, c("vendor_name", "vendor_short"), with=FALSE])
v_names <- rbindlist(list(v_names, tmp_v_names))
v_names <- unique(v_names[vendor_short!=vendor_name])
rm(tmp_v_names)
...
scatter_dt <- ir_detail[vendor_short %in% ir_summary_red[, vendor_short]][mm_ir_cases>0]
...
p_scatter <- ggplot(data=scatter_dt, aes(x=vendor_short, y=num_IRs)) +
  ...
  coord_flip() + my_theme +
  theme(plot.title = element_text(size=12, face="bold", vjust=-0.1),
...
### make as concise as possible ... narrow dimensions preferred ###
...
fy_summary_red[, v_short := shortVendor(vendor_short)]
setcolorder(fy_summary_red,
  c("vendor_short", "v_short", "qtr", "num_authorized",
    "num_ir_cases", "num_IRs", "pct_con_IRs", "pct_label"))
fy_summary_red[, qtr := factor(qtr, levels =rev(sort(as.character(unique(qtr)))))]

vendor_list <- fy_summary_red[, unique(v_short)]
...
fy_a_dt <- copy(fy_summary_red[v_short %in% vendors_l1])
fy_a_dt[, label_length := nchar(pct_label)/max(pct_con_IRs, na.rm=TRUE)*2.5+pct_con_IRs]
fy_b_dt <- copy(fy_summary_red[v_short %in% vendors_l2])
fy_b_dt[, label_length := nchar(pct_label)*170/max(pct_con_IRs, na.rm=TRUE)+pct_con_IRs]

fy_one_plot_a <- ggplot(data=fy_a_dt, position=position_dodge(0.8),
       aes(x=v_short, y=pct_con_IRs, ymax=pct_con_IRs*1.19, fill=qtr))+
  geom_bar(stat="identity", width=0.8, size=0.1, color="black", position=position_dodge(0.8))+
  geom_point(stat="identity", width=0.8, color="black", size=5.5, position=position_dodge(0.8),
             aes(x=v_short, y=label_length), show_guide=FALSE)+
  geom_point(stat="identity", width=0.8, color="white", size=4.5, position=position_dodge(0.8),
             aes(x=v_short, y=label_length), show_guide=FALSE)+
  coord_flip()+
  my_theme+theme(plot.title = element_text(size=10, face="bold", vjust=-0.2),
                 legend.margin=unit(-0.6,"cm"),
                 legend.position="top", legend.key.size = unit(0.1, "inch"),
  ...
  geom_text(data=fy_a_dt, position=position_dodge(0.8),
    aes(x=v_short, y=pct_con_IRs, fill=qtr, label=pct_label), hjust=0, size=2.5)+
  geom_text(data=fy_a_dt, position=position_dodge(0.8),
            aes(x=v_short, y=label_length, label=num_IRs, fill=qtr), color="black", size=2.5)+
  labs(title=paste0(qtr_dt_list[i, qtr],
    ": IRs - Missed Meds - Group 1\n (# IR missed meds consumers/# authorized), total IRs circled"),
...
mm_plots[[length(mm_plots)+1]] <- fy_one_plot_a

fy_one_plot_b <- ggplot(data=fy_b_dt, position=position_dodge(0.8),
                        aes(x=v_short, y=pct_con_IRs, ymax=pct_con_IRs*1.19, fill=qtr))+
  geom_bar(stat="identity", width=0.8, size=0.1, color="black", position=position_dodge(0.8))+
  geom_point(stat="identity", width=0.8, color="black", size=5.5, position=position_dodge(0.8),
             aes(x=v_short, y=label_length), show_guide=FALSE)+
  ...
  my_theme+theme(plot.title = element_text(size=10, face="bold", vjust=-0.2),
                 ...
                 axis.text.y = element_text(size=6),
                 plot.margin = unit(c(0.2,0.2,0.2,0.2), "cm"))+
  geom_text(data=fy_b_dt, position=position_dodge(0.8),
            aes(x=v_short, y=pct_con_IRs, fill=qtr, label=pct_label), hjust=0, size=2.5)+
  geom_text(data=fy_b_dt, position=position_dodge(0.8),
            aes(x=v_short, y=label_length, label=num_IRs, fill=qtr), color="black", size=2.5)+
  ...
  guides(fill=guide_legend(reverse=TRUE),
         colour=guide_legend(reverse=TRUE))
mm_plots[[length(mm_plots)+1]] <- fy_one_plot_b



fy_summary_red[, label_length := nchar(pct_label)/max(pct_con_IRs, na.rm=TRUE)*2.5+pct_con_IRs]
fy_summary_red[, label_length := nchar(pct_label)*170/max(pct_con_IRs, na.rm=TRUE)+pct_con_IRs]
fy_facets <- ggplot(data=fy_summary_red, position=position_dodge(0.8),
       aes(x=v_short, y=pct_con_IRs, ymax=pct_con_IRs*2.5, fill=qtr))+
  geom_bar(stat="identity", width=0.8, size=0.1, color="black", position=position_dodge(0.8))+
  geom_point(stat="identity", width=0.8, color="black", size=5.5, position=position_dodge(0.8),
             aes(x=v_short, y=0), show_guide=FALSE)+
  geom_point(stat="identity", width=0.8, color="white", size=4.5, position=position_dodge(0.8),
             aes(x=v_short, y=0), show_guide=FALSE)+
  facet_wrap(~v_short, scales="free", ncol=6)+
  my_theme+theme(plot.title = element_text(size=11, face="bold", vjust=-0.2),
                 ...
                 axis.title.y = element_blank(),
                 plot.margin = unit(c(0.2,0.2,0.2,0.2), "cm"))+
  geom_text(data=fy_summary_red, position=position_dodge(0.8),
            aes(x=v_short, y=pct_con_IRs, fill=qtr, label=pct_label), hjust=0, size=2.5, angle=90)+
  geom_text(data=fy_summary_red, position=position_dodge(0.8),
            aes(x=v_short, y=0, label=num_IRs, fill=qtr), color="black", size=2.5, angle=90)+
  labs(title=paste0(qtr_dt_list[i, qtr],
...
for(i in (nrow(qtr_dt_list)+1) : (nrow(qtr_dt_list)+2)) {
  mydoc <- addPlot( mydoc, fun=print, x=mm_plots[[i]], width=6, height=5.25 )
}
mydoc <- addPlot( mydoc, fun=print, x=mm_plots[[6]], width=6, height=5.5 )

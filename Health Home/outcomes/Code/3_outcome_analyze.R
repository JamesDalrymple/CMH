...
analyze$bmi_team$pct <- analyze$bmi_team$table[,
  list(improved = round(improved/total*100, 1),
       maintained = round(maintained/total*100, 1),
       regressed = round(regressed/total*100, 1),
       unknown = round(unknown/total*100, 1)), by = team]
...
analyze$bmi_hh$pct <- analyze$bmi_hh$table[,
  list(improved = round(improved/total*100, 1),
       maintained = round(maintained/total*100, 1),
       regressed = round(regressed/total*100, 1),
       unknown = round(unknown/total*100, 1)), by = hh_team]
analyze$bmi_hh$pct <-

...
height <- copy(sql$height)
height[, h_inch := height_feet*12+height_inches]
height <- height[!(h_inch < 3*12 | h_inch > 8*12)]
# median height is safest
modify$h_agg <- height[,
  list(h_median = median(h_inch)
       # h_mean = mean(h_inch, trim = 0.35), samples = .N
       ), by = case_no]
modify$h_agg[, Cs(h_mean, samples) := NULL]
...
output <- output[age >= 17] # must be 18+ on every vt_date
...
modify$bmi_dt[modify$h_agg, h_median := i.h_median, on = "case_no"]
modify$bmi_dt[total_inches==0 & !is.na(h_median), total_inches := h_median]
modify$bmi_dt[, h_median := NULL]
# fix errors manually ---------------------------------------------------------
modify$bmi_dt[case_no == 10735 & vt_date == "2015-01-26", weight := 130]
modify$bmi_dt[case_no == 12462 & vt_date == "2015-02-26", weight := 160]
modify$bmi_dt[case_no == 209614 & vt_date == "2014-12-02", weight := 140]
modify$bmi_dt[case_no == 99374 & vt_date == "2014-11-18", weight := 210]
...
saved$bmi_too_low_records <-
  modify$bmi_dt[case_no %in% modify$bmi_dt[bmi < 15 & max_vt_date == vt_date,
                             unique(case_no)],
...
modify$bmi_dt <- modify$bmi_dt[case_no %nin%
  modify$bmi_dt[bmi < 15 & max_vt_date == vt_date, unique(case_no)]]
...
saved$bad_weight <- modify$bmi_dt[bmi < 15 & weight < 40, unique(.SD),

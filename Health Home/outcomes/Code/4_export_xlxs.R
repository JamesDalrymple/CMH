...
# bmi raw data
bmi_raw_sheet <- createSheet(wb, sheetName = "bmi_raw")
addDataFrame(x = saved$bmi, sheet = bmi_raw_sheet, showNA = FALSE,
  row.names = FALSE, startRow = 1, startColumn = 1, colnamesStyle = cs3)
# bmi pre/post data
bmi_pp_sheet <- createSheet(wb, sheetName = "bmi_pre_post")
addDataFrame(x = modify$bmi_pre_post, sheet = bmi_pp_sheet, showNA = FALSE,
             row.names = FALSE, startRow = 1, startColumn = 1,
             colnamesStyle = cs3)
# wn raw data
wn_raw_sheet <- createSheet(wb, sheetName = "wn_raw")
addDataFrame(x = saved$wn, sheet = wn_raw_sheet, showNA = FALSE,
  row.names = FALSE, startRow = 1, startColumn = 1, colnamesStyle = cs3)
# wn post/pre data
wn_pp_sheet <- createSheet(wb, sheetName = "wn_pre_post")
addDataFrame(x = modify$wn_pre_post, sheet = bmihh_sheet, showNA = FALSE,
  row.names = FALSE, startRow = 1, startColumn = 1, colnamesStyle = cs3)
# BMI improvement
bmi_imp_sheet <- createSheet(wb, sheetName = "bmi_imp")
addDataFrame(x = saved$bmi_imp, sheet = bmi_imp_sheet, showNA = FALSE,
  row.names = FALSE, startRow = 1, startColumn = 1, colnamesStyle = cs3)
# overal health improvement
ovr_imp_sheet <- createSheet(wb, sheetName = "ovr_imp")
addDataFrame(x = saved$health_imp, sheet = ovr_imp_sheet, showNA = FALSE,
             row.names = FALSE, startRow = 1, startColumn = 1, colnamesStyle = cs3)
# summary data
summary_sheet <- createSheet(wb, sheetName = "summary")
addDataFrame(x = saved$hh_summary, sheet = summary_sheet, showNA = FALSE,
  row.names = FALSE, startRow = 1, startColumn = 1, colnamesStyle = cs3)
...
saveWorkbook(wb = wb, file = file.path(project_wd$results,
  paste0("outcomes_data_analyis ", format(input$run_date, "%b%Y"), ".xlsx"))
...
<<initialize, cache=FALSE, results="hide", echo=FALSE, warnings=FALSE, message=FALSE, tidy=TRUE>>=
  suppressWarnings(source(file.path(gsub(x=getwd(), pattern="/LaTeX", replace=""),
                   "monthly pi_v3.r")))
@

\lhead{Monthly Management Report - \Sexpr{current_month} }
\cfoot{CSTS \Sexpr{current_month} Monthly PI Report - \textit{Powered by R and LaTeX} - Page \thepage\ of \pageref{LastPage}}
...
Unsigned/Draft Documents by Supervisor/Enterer - Report 2004: ran \Sexpr{gsub(x=run_date, pattern="_", replace="-")} \newline
\small{
...
Caseload and Not Seen 30/60/90/180 days (Report 2044: ran \Sexpr{gsub(x=run_date, pattern="_", replace="-")})(No services at all) \newline
\small{
<<cl_t1, echo=FALSE, results='asis'>>=
    tmpDT <- svc_table
    setnames(tmpDT, old=c("primary_staff", "case_load", "notSeen30days", "notSeen60days", "notSeen90days", 'notSeen180days'),

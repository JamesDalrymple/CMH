...
codeWD <- "Dropbox/PI Projects/Monthly Management/R Code"
source(file.path(baseWD, codeWD, "pi auxillary.r"))
#### user input required ####
# current month
current_month <- "December"
current_fy <- "2016"
run_date <- "1_4_2016"
report_end <- gsub(run_date, pattern="_", replace="/")
...
# if month == "October", need to make new files and take away warnings
# from making the year holder files (i.e. ipos fy 2016.csv)
if(current_month %in% c("October", "November", "December")) {
  calendar_year <- as.character(as.numeric(current_fy)-1)
} else { calendar_year <- current_fy}
...
if(!dir.exists(file.path(baseWD, resultsWD, current_fy, current_month))) {
  ...
  print(paste("created directory:", file.path(baseWD, resultsWD, current_fy, current_month)))
}
if(!dir.exists(file.path(baseWD, dataWD, current_fy, current_month))) {
  # load most recent teamStaffLevels.csv
  fy_mons_completed <- grep(x=list.files(file.path(baseWD, dataWD, current_fy)), pattern=".csv", invert = TRUE, value=TRUE)
  recent_mon <- (as.yearmon(paste("Oct", as.numeric(calendar_year)-1))+
                   0:(length(fy_mons_completed)-1)/12)[length(fy_mons_completed)]
  dir.create(file.path(baseWD, dataWD, current_fy, current_month))
  print(paste("created directory:", file.path(baseWD, dataWD, current_fy, current_month)))
  if(length(fy_mons_completed) > 0) {

    team_staff_levels <- read.dtable(list.files(list.files(file.path(baseWD, dataWD, current_fy),
      pattern=substr(recent_mon, 1, 3), full.names = TRUE), pattern="team", full.names = TRUE))
  } else {
    team_staff_levels <- read.dtable(list.files(list.files(file.path(baseWD, dataWD, as.numeric(current_fy)-1),
      pattern="September", full.names = TRUE), pattern="team", full.names = TRUE))

  }
  write.csv(team_staff_levels, file.path(baseWD, dataWD, current_fy, current_month, "teamStaffLevels.csv"), row.names = FALSE)
  print(paste("team_staff_levels added:", file.path(baseWD, dataWD, current_fy, current_month, "teamStaffLevels.csv")))
}

currentDay <- as.Date(run_date, format="%m_%d_%Y")
fy_start <- as.Date(as.yearmon(paste("Oct", as.numeric(current_fy)-1)))
fy_file<- format(fy_start, "%m_%d_%Y")
fy_file <- gsub(x=fy_file, pattern="_01_", replace="_1_", fixed=TRUE)
...
num_months <- (as.yearmon(paste(current_month, calendar_year))-
               as.yearmon(paste("October", as.numeric(current_fy)-1 )))*12+1
...
case_load <- sqlQuery(channel=channel, query=sql_case_load, stringsAsFactors=FALSE)
...
sql_ipos <- "select distinct
  ...
  'WSH - ACT' , 'WSH - Children''s Services' ,'WSH - Children''s Services - Home Based',
  'WSH - MI - Adult', 'WSH - DD Adult')"
ipos <- sqlQuery(channel=channel, query=sql_ipos, stringsAsFactors=FALSE)
...
last_svc <- sqlQuery(channel = channel, query=sql_last_svc, stringsAsFactors=FALSE)
last_svc <- data.table(last_svc)
# demographic errors
sql_demo_errors <- paste0("select distinct
	team, case_no, num_qi_errors, supervisor, primary_staff
  from encompass.dbo.E2_Fn_QI_Demo_Errors2('Washtenaw', '", fy_start, "' , '", report_end, "') as qi")
demo_errors <- sqlQuery(channel = channel, query=sql_demo_errors, stringsAsFactors=FALSE)
...
health_errors <- sqlQuery(channel = channel, query=sql_health_errors, stringsAsFactors=FALSE)
health_errors <- data.table(health_errors)
# wage errors
sql_wage_errors <- paste0("select distinct
  ...
  from encompass.dbo.E2_Fn_Min_Wage_Missed('Washtenaw', '", fy_start, "' , '", report_end, "') as qi
...
wage_errors <- sqlQuery(channel = channel, query=sql_wage_errors, stringsAsFactors=FALSE)
wage_errors <- data.table(wage_errors)
# unsigned drafts by supervisor and author
sql_unsigned_docs <- paste0("select distinct
  ...
  from encompass.dbo.E2_Fn_Draft_Documents_by_Staff_Date3 ('Washtenaw',  '", fy_start, "' , '", report_end, "' ) doc
  group by DO_TITLE,
	...
	doc.FaceToFace")
unsigned_docs <- sqlQuery(channel = channel, query=sql_unsigned_docs, stringsAsFactors=FALSE)
unsigned_docs <- data.table(unsigned_docs)
# self sufficiency matrix
sql_ssm_summary <- paste0("select supervisor, at_least2_ssms, case_load
  from James_CSTS.dbo.JD_E2_fn_MI_atleast2_SSM('3/1/2014', '", report_end, "' )
  order by at_least2_ssms desc")
ssm_summary <- sqlQuery(channel = channel, query=sql_ssm_summary,
                        stringsAsFactors=FALSE)
ssm_summary <- data.table(ssm_summary)
sql_smm_case_detail <- paste0("select distinct supervisor, primary_staff, case_no, at_least2_ssms
  from dbo.JD_E2_fn_MI_SSM_detail('3/1/2014', '", report_end, "')
  order by supervisor, Primary_Staff, case_no")
smm_case_detail <- sqlQuery(channel = channel, query=sql_smm_case_detail, stringsAsFactors=FALSE)
smm_case_detail <- data.table(smm_case_detail)
sql_ssm_staff_detail <- paste0("select
...
ssm_staff_detail <- sqlQuery(channel = channel, query=sql_ssm_staff_detail, stringsAsFactors=FALSE)
ssm_staff_detail <- data.table(ssm_staff_detail)
teamLevels <- read.dtable(file.path(baseWD, dataWD, current_fy, current_month,
                                    "teamStaffLevels.csv"))

# temp notes:
  # case_load replaces case_load
  # ipos
  # last_svc replaces lastSvc
  # demo_errors replaces demoError
  # health_errors replaces health_errors
  # wage_errors replaces wageError
  # unsigned_docs replaces unsignSup
  # ssm_summary replaces ssm
#### team levels ####
teamLevels[, team := teamFix(team)]
setnames(teamLevels, "entered_by", "author")
teamOnly <- teamLevels[author=="all"]
staffOnly <- teamLevels[author!="all"]
#### case load ####
# fix teams
case_load[, team := teamFix(team)]
# keep only CSTS teams
case_load <- setkey(case_load, team)[J(teamsCMH)]
conTeams <- case_load[, list(case_load = length(unique(case_no))), by = team]
#### last service ####
# fix teams
last_svc[, team := teamFix(team)]
...
services_list <- last_svc[, .SD, .SDcols = c("team", "primary_staff", "case_no",
      "num_days_no_service", "notSeen90days", "notSeen180days")]
services_list <- merge(services_list, unique(case_load[, .SD, .SDcols = c("supervisor", "primary_staff")]), by="primary_staff")
setcolorder(services_list, c("team", "supervisor", "primary_staff", "case_no", "num_days_no_service", "notSeen90days", "notSeen180days"))
services_list <- services_list[notSeen90days==1 | notSeen180days==1]
#### current IPOS ####
# fix teams
ipos[, team := teamFix(team)]
...
setkey(ipos, ipos_type)[J("Interim IPOS"),  interimDiff :=  pmin(ip_expdt, as.Date(Sys.time()))-ip_effdt]
ipos[, ipos30Plus := 0]
...
missIPOS <- setkey(ipos, ipos_type)[J(""), list(missingIPOS=length(unique(case_no))), by=team]
invisible(setkey(ipos, NULL))
conIPOS <- ipos[, list(totalConsumers=length(unique(case_no))), by=team]
# which consumers have a current IPOS
ipos[, current := 0]
ipos[currentDay <= ip_expdt, current := 1]
currentIPOS <- ipos[, list(currentIPOS = sum(current, na.rm=TRUE)), by=team]
# which consumers have an expired IPOS
expireIPOS <- ipos[, list(expiredIPOS = sum(current, na.rm=TRUE)), by=list(team, case_no)]
expireIPOS <- setkey(expireIPOS, expiredIPOS)[J(0)]
expireIPOS <- expireIPOS[, list(expiredIPOS = length(unique(case_no))), by=team]
# which consumers have a full IPOS
ipos_full <- setkey(ipos, ipos_type)[J("Full IPOS"), list(Full_IPOS = length(ip_effdt)), by=team]
...
ipos_interim <- setkey(ipos, ipos_type)[J("Interim IPOS"), list(Interim_IPOS = length(ip_effdt)), by=team]
...
ipos_interim_30_plus <- ipos[, list(Interim_IPOS_Over_30_Days = sum(ipos30Plus, na.rm=TRUE)), by=team]
ipos_interim_30_plus <- setkey(ipos_interim_30_plus, team)[J(teamsCMH)]
ipos_interim_30_plus[is.na(ipos_interim_30_plus)] <- 0
# which consumers have an Preliminary IPOS
ipos_prelim <- setkey(ipos, ipos_type)[J("Preliminary IPOS"), list(Preliminary_IPOS = length(ip_effdt)), by=team]
...
ipos_single <- setkey(ipos, ipos_type)[J("Single Service IPOS"), list(Single_Service_IPOS = length(ip_effdt)), by=team]
...
iposTeam <- mergeDT( DT=list(conIPOS, currentIPOS, missIPOS, expireIPOS), all.x=TRUE, by="team")
...
# keep only CMH teams
iposTeam <- iposTeam[J(teamsCMH)]
### the table of different types of IPOS ###
# part a - grand totals
iposTeam2a <- mergeDT( DT=list(conIPOS, currentIPOS), all=TRUE, by="team")
iposTeam2a <- setkey(iposTeam2a, team)[J(teamsCMH)]
# part b - all ipos types
iposTeam2b <- setkey(ipos, team)[teamsCMH, list(currentIPOS = sum(current, na.rm=TRUE)), by=list(team, ipos_type)]
iposTeam2b <- dcast.data.table(iposTeam2b, formula = team ~ ipos_type,
                               value.var="currentIPOS", fill=0)
# part c - blank/missing and expired IPOS
iposTeam2cExpire <- ipos[ipos_type!="", list(expired = length(current)-sum(current)), by=list(team, case_no)]
iposTeam2cExpire <- iposTeam2cExpire[, list(expiredIPOS = sum(expired)), by=team]
iposTeam2cBlank <- ipos[ipos_type=="", list(blank_missing_IPOS = length(unique(case_no))), by = team]
iposTeam2cBlank <- setkey(iposTeam2cBlank, team)[teamsCMH, nomatch=NA]
iposTeam2cBlank[is.na(iposTeam2cBlank)] <- 0
iposTeam2c <- mergeDT(DT=list(iposTeam2cBlank, iposTeam2cExpire), by="team", all.x=TRUE)
# combine parts a-c
iposTeam2 <- mergeDT(DT = list(iposTeam2a, iposTeam2b, iposTeam2c), by="team", all=TRUE)
### IPOS tables ###
## ipos table 1
ipos_table1 <- mergeDT(DT = list(ipos_full, ipos_interim, ipos_interim_30_plus, ipos_prelim, ipos_single), by="team" )
ipos_table1[, Grand_Total := psum(Full_IPOS, Interim_IPOS, Preliminary_IPOS, Single_Service_IPOS)]
ipos_table1 <- cbind(month=as.character(as.yearmon(paste(current_month, calendar_year))), ipos_table1)
## ipos table 2
ipos_table2 <- mergeDT(DT = list(currentIPOS, iposTeam2c), by="team")
ipos_table2 <- cbind(month=as.character(as.yearmon(paste(current_month, calendar_year))), ipos_table2)
## ipos table 3
ipos_table3 <- ipos[current==0 | ipos_type=="",
                    list(expired_missing_IPOS = length(unique(case_no))), by=list(team, supervisor, primary_staff)]
...
ipos_table3 <- rbindlist( list(
  ...
  ipos_table3))
ipos_table3 <- ipos_table3[order(team, supervisor, primary_staff, expired_missing_IPOS)]
#### unsigned/draft Documents ####
# keep only Draft and Unsigned
unsigned_docs <- setkey(unsigned_docs, doc_status, f2f)[J(c("Draft", "Unsigned"), "Y")]
invisible(setkey(unsigned_docs, NULL))
# keep only wanted documents
unsigned_docs <- merge(unsigned_docs, data.table(do_title = docsKeep), by="do_title", all.x=TRUE)
# fix team column
...
## assign levels ##
unsignTeam2 <- mergeDT( DT=list(unsignTeam2, teamOnly[, c("team", "level"), with=FALSE]),
                        by=c("team"), all.x=TRUE)
unsignTeam2 <- mergeDT( DT=list(unsignTeam2, staffOnly),
                        by=c("team", "author"), all.x=TRUE)
...
unsign_t1a <- unsignTeam2[, list(author = "",
                                 unsigned_draft_docs = sum(unsigned_draft_docs, na.rm=TRUE)), by=list(supervisor)]
unsign_t1b <- unsignTeam2[, list(unsigned_draft_docs = sum(unsigned_draft_docs, na.rm=TRUE)), by=list(supervisor, author)]
...
unsign_table <- rbindlist(list(unsign_table,
                               data.table(supervisor="\\textbf{Grand total}", author="", unsigned_draft_docs =
                                            paste0("\\textbf{", unsignTeam2[, sum(unsigned_draft_docs)], "}") )))
#### consumers with Demographic Errors ####
## demo_error list ##
demo_errors[, team := teamFix(team)]
demo_error_list <- demo_errors[num_qi_errors>0][order(team, supervisor, primary_staff)]
demo_error_list <- setkey(demo_error_list, team)[J(teamsCMH), nomatch=0]
# fix teams
...
colOrder <- c("team", "primary_staff_supervisor", "primary_staff", "case_no", "data_issue_type", "notes")
...
health_error_list <- setkey(health_error_list, team)[J(teamsCMH), nomatch=0]
...
# aggregate data
health_team <- health_errors[, list(con_errors = length(unique(case_no))), by=list(team)]
...
setcolorder(wage_errors, c("case_no", "team", "supervisor", "primary_staff", "employment_status", "minimum_wage_error", "minimum_wage"))
## list of ordered errors
wage_error_list <- wage_errors[minimum_wage_error>0][order(team, supervisor, primary_staff)]
wage_error_list <- setkey(wage_error_list, team)[J(teamsCMH), nomatch=0]
# keep wanted columns
...
# aggregate data
wageTeam <- wage_errors[, list(con_errors = sum(minimum_wage_error, na.rm=TRUE)), nomatch=NA, by=team]
...
setnames(ssm_summary, old=names(ssm_summary), new=gsub(x=tolower(names(ssm_summary)), pattern="[.]", replace="_") )
setnames(ssm_summary, old=grep(x=names(ssm_summary), pattern="supervisor", value=TRUE), new="supervisor" )
...
svcTeamStaff <- setkey(last_svc, team)[J(teamsCMH), list(case_load = length(unique(case_no)),
  ...
  notSeen180days = sum(notSeen180days, na.rm=TRUE)), by=list(team, primary_staff)]
## service by team ##
svcTeam <- setkey(last_svc, team)[J(teamsCMH), list(case_load = length(unique(case_no)),
  primary_staff = "", level = "top",
  notSeen30days = sum(notSeen30days, na.rm=TRUE),
  notSeen60days = sum(notSeen60days, na.rm=TRUE),
  notSeen90days = sum(notSeen90days, na.rm=TRUE),
  notSeen180days = sum(notSeen180days, na.rm=TRUE)), by=team]
## service - grand total ##
...
svcTeam2 <- setkey(last_svc, team)[J(teamsCMH), list(case_load = length(unique(case_no)),
  notSeen30days = sum(notSeen30days, na.rm=TRUE),
  notSeen60days = sum(notSeen60days, na.rm=TRUE),
  notSeen90days = sum(notSeen90days, na.rm=TRUE),
...
svcTeam2 <- merge(svcTeam2, staffOnly[, c("primary_staff", "level"), with=FALSE], all.x=TRUE, by="primary_staff")
setkey(svcTeam2, team)[J("Child"), level := "3"]
setkey(svcTeam2, team)[J(c("ACT", "Child Home Based")), level := "4"]
svcTeam2[is.na(level), level := "-"]
setcolorder(svcTeam, c("team", "primary_staff", "case_load", "level", "notSeen30days", "notSeen60days", "notSeen90days", "notSeen180days"))
setcolorder(svcTeam2, c("team", "primary_staff", "case_load", "level", "notSeen30days", "notSeen60days", "notSeen90days", "notSeen180days"))
svc_table <- rbindlist(list(svcTeam, svcTeam2))
svc_table[level=="", level := "-"]
svc_table[, level := factor(level, levels=c("top", "DM", "respite", "Supervisor", "Therapy", 5:1, "-") )]
...
### ipos results ###
iposGraph <- iposTeam[, list(team=team,
...
## remove current month, add it back in to make sure current month is
  # most up-to-date, then save ##
if(!file.exists(file.path(baseWD, dataWD, current_fy,
                           paste0("ipos fy ", current_fy, ".csv")) )) {
  write.csv(iposGraph[order(team, mon_fy)],
            file.path(baseWD, dataWD, current_fy,
              paste0("ipos fy ", current_fy, ".csv") ), row.names=FALSE)
  warning(paste("A new file was created:",
    file.path(baseWD, dataWD, current_fy,
              paste0("ipos fy ", current_fy, ".csv"))))
}
iposAllMonths <- read.dtable(file.path(baseWD, dataWD, current_fy, paste0("ipos fy ", current_fy, ".csv")))
iposAllMonths[, mon_fy := as.yearmon(gsub(x=mon_fy, pattern="-", replace=" 20"))]
iposAllMonths <-
  setkey(iposAllMonths, mon_fy)[
    !J(as.yearmon(paste(current_month, calendar_year) ))]
invisible(setkey(iposAllMonths, NULL))
iposGraph <- rbindlist(list(iposAllMonths, iposGraph))
rm(iposAllMonths)
iposGraph <- unique(iposGraph)
write.csv(iposGraph[order(team, mon_fy)],
          file.path(baseWD, dataWD, current_fy,  paste0("ipos fy ", current_fy, ".csv") ), row.names=FALSE)

## ipos_table1
if(!file.exists( file.path(baseWD, dataWD, current_fy,
                           paste0("ipos t1 fy ", current_fy, ".csv")) )) {
  write.csv(ipos_table1, file.path(baseWD, dataWD, current_fy,
                                   paste0("ipos t1 fy ", current_fy, ".csv")), row.names=FALSE)
  warning(paste("A new file was created:", file.path(baseWD, dataWD, current_fy,
                                                     paste0("ipos t1 fy ", current_fy, ".csv")) ))
}
full_ipos_t1 <- read.dtable(file.path(baseWD, dataWD, current_fy,
                                      paste0("ipos t1 fy ", current_fy, ".csv") ))
full_ipos_t1[, month := as.character(as.yearmon(gsub(x=month, pattern="-", replace=" 20")))]
full_ipos_t1 <- setkey(full_ipos_t1, month)[!J(as.character(as.yearmon(paste(current_month, calendar_year) )))]
...
write.csv(full_ipos_t1[order(month, team)],
          file.path(baseWD, dataWD, current_fy, paste0("ipos t1 fy ", current_fy, ".csv")), row.names=FALSE)
full_ipos_t1 <- read.dtable(file.path(baseWD, dataWD, current_fy, paste0("ipos t1 fy ", current_fy, ".csv")))
full_ipos_t1 <- full_ipos_t1[order(team, as.yearmon(month))]
setnames(full_ipos_t1, names(full_ipos_t1), gsub(x=names(full_ipos_t1), pattern="_", replace=" ") )

## ipos_table2
if(!file.exists( file.path(baseWD, dataWD, current_fy,
                           paste0("ipos t2 fy ", current_fy, ".csv")) )) {
  write.csv(ipos_table2, file.path(baseWD, dataWD, current_fy,
                                   paste0("ipos t2 fy ", current_fy, ".csv")), row.names=FALSE)
  warning(paste("A new file was created:", file.path(baseWD, dataWD, current_fy,
                                                     paste0("ipos t2 fy ", current_fy, ".csv")) ))
}
full_ipos_t2 <- read.dtable(file.path(baseWD, dataWD, current_fy,
                                      paste0("ipos t2 fy ", current_fy, ".csv")))
full_ipos_t2[, month := as.character(as.yearmon(gsub(x=month, pattern="-", replace=" 20")))]
full_ipos_t2 <- setkey(full_ipos_t2, month)[!J(as.character(as.yearmon(paste(current_month, calendar_year))))]
full_ipos_t2 <- rbindlist(list(full_ipos_t2, ipos_table2))
write.csv(full_ipos_t2[order(team, month)], file.path(baseWD, dataWD, current_fy,
                                                      paste0("ipos t2 fy ", current_fy, ".csv")), row.names=FALSE)
full_ipos_t2 <- read.dtable(file.path(baseWD, dataWD, current_fy,
                                      paste0("ipos t2 fy ", current_fy, ".csv")))
...
setnames(full_ipos_t3, old=c("primary_staff", "expired_missing_IPOS"), new=c("primary_staff", "expired/missing IPOS"))
setcolorder(full_ipos_t3, c("team", "supText", "supervisor", "primary_staff", 'expired/missing IPOS'))

# reshape/melt data for ggplot
iposGraph <- data.table(
  reshape::melt(iposGraph, id=c("team", "mon_fy"), variable.names=c("pctCurrentIPOS", "missingIPOS"))
)
...
iposGraph <- merge(all_combinations, iposGraph, all=TRUE, by=c("team", "mon_fy"))
# factor based on 12 consecutive months
iposGraph[, mon_fy := factor(mon_fy, levels = as.character(as.yearmon(fy_start)+0:11/12))]
...
for(i in seq_along(team_list)) {
  # start for loop
  hist_ipos[[i]] = ggplot(data=iposGraph[team==team_list[i]], aes(x=mon_fy, y=value, fill=variable, ymax=1.25*value))+
    geom_bar(width=0.3, stat="identity", color="black", size=1, position=position_dodge(0.3))+
    labs(title = paste0(team_list[i], " Graph ", i, ".1.1 - % Current IPOS (Report 2003: ran ",
                        gsub(x=run_date, pattern="_", replace="-"), ")"),
         y="percent", fill="")+
    geom_text(data=iposGraph[team==team_list[i]], position=position_dodge(0.3),
...
if(!file.exists( file.path(baseWD, dataWD, current_fy,
  ...
  write.csv(unsignTeam, file.path(baseWD, dataWD, current_fy,
                                  paste0("unsign_draft_docs t1 fy ", current_fy, ".csv")), row.names=FALSE)
  warning(paste("A new file was created:", file.path(baseWD, dataWD, current_fy,
                                                     paste0("unsign_draft_docs t1 fy ", current_fy, ".csv")) ))
}
unsign_draft_t1 <- read.dtable(file.path(baseWD, dataWD, current_fy,  paste0("unsign_draft_docs t1 fy ", current_fy, ".csv")))
unsign_draft_t1[, month := as.character(as.yearmon(gsub(x=month, pattern="-", replace=" 20")))]
unsign_draft_t1 <- setkey(unsign_draft_t1, month)[!J(as.character(as.yearmon(paste(current_month, calendar_year) )))]
unsign_draft_t1 <- rbindlist(list(unsign_draft_t1, unsignTeam))
write.csv(unsign_draft_t1[order(team, as.yearmon(month))],
          file.path(baseWD, dataWD, current_fy,  paste0("unsign_draft_docs t1 fy ", current_fy, ".csv")), row.names=FALSE)
unsign_draft_t1 <- read.dtable(file.path(baseWD, dataWD, current_fy,  paste0("unsign_draft_docs t1 fy ", current_fy, ".csv")))

# re-name to match
setnames(all_combinations, old="mon_fy", new="month")
# merge with all possibilities
unsign_draft_t1 <- merge(all_combinations, unsign_draft_t1, all=TRUE, by=c("team", "month"))
# re-order - may not be necessary
unsign_draft_t1 <- unsign_draft_t1[order(team, as.yearmon(month))]
# factor based on 12 consecutive months
unsign_draft_t1[, month := factor(month, levels = as.character(as.yearmon(fy_start)+0:11/12))]

p_unsign <- list()
for(i in seq_along(team_list)) {
  # create graph p_unsign
  p_unsign[[i]] = ggplot(data=unsign_draft_t1[team==team_list[i]], aes(x=month, fill=month, y=docs_unsigned, ymax=1.25*docs_unsigned))+
    geom_bar(width=0.5, stat="identity", color="black", size=1, fill="#C282D6", position=position_dodge(0.5))+
    labs(title = paste0(team_list[i], " Graph ", i, ".2.1 - # Unsigned/Draft Docs (Report 2004: ran ",
                        gsub(x=run_date, pattern="_", replace="-"), ")"),
         y = "# unsigned/draft documents")+
    geom_text(vjust=0.5, hjust=-0.2, aes(x=month, fill=month, y=docs_unsigned, label=docs_unsigned),
...
if(!file.exists( file.path(baseWD, dataWD, current_fy,
  ...
  write.csv(demo_team[order(team, month)], file.path(baseWD, dataWD, current_fy,
                                                     paste0("demo table fy ", current_fy, ".csv") ), row.names=FALSE)
  warning(paste("A new file was created:", file.path(baseWD, dataWD, current_fy,
                                                     paste0("demo table fy ", current_fy, ".csv"))))
}
demo_table <- read.dtable(file.path(baseWD, dataWD, current_fy,
                                    paste0("demo table fy ", current_fy, ".csv")))
demo_table[, month := as.character(as.yearmon(gsub(x=month, pattern="-", replace=" 20")))]
demo_table <- setkey(demo_table, month)[!J(as.character(as.yearmon(paste(current_month, calendar_year))))]
setcolorder(demo_team, names(demo_table))
demo_table <- rbindlist(list(demo_table, demo_team))
write.csv(demo_table[order(team, as.yearmon(month))], file.path(baseWD, dataWD, current_fy,
                                                                paste0("demo table fy ", current_fy, ".csv")), row.names=FALSE)
demo_table <- read.dtable(file.path(baseWD, dataWD, current_fy,
                                    paste0("demo table fy ", current_fy, ".csv")))
demo_table[is.na(con_errors), con_errors := 0]
# merge with all possibilities
demo_table <- merge(all_combinations, demo_table, all=TRUE, by=c("team", "month"))
# re-order - may not be necessary
demo_table <- demo_table[order(team, as.yearmon(month))]
# factor based on 12 consecutive months
demo_table[, month := factor(month, levels = as.character(as.yearmon(fy_start)+0:11/12))]

p_demo <- list()
for(i in seq_along(team_list)) {
  # create graph p_demo
  p_demo[[i]] = ggplot(data=demo_table[team==team_list[i]], aes(x=month, fill=month, y=con_errors, ymax=1.35*con_errors))+
    geom_bar(width=0.5, stat="identity", color="black", size=1, fill="#C282D6", position=position_dodge(0.5))+
    labs(title = paste0(team_list[i], " Graph ", i, ".3.1 - # Consumers with Demographic Errors\n (Report 2023: ran ",
                        gsub(x=run_date, pattern="_", replace="-"), ")"),
         y="# consumer errors")+
    geom_text(vjust=0.5, hjust=-.5, aes(x=month, fill=month, y=con_errors, label=con_errors),
              angle=90, position=position_dodge(0.5), size=2.75, na.rm=TRUE)+
    my_theme+
    theme(legend.position="top",
          panel.grid.major = element_line(colour = "grey80", size = 0.2),
...
if(!file.exists( file.path(baseWD, dataWD, current_fy,
  ...
  write.csv(health_team[order(team, month)], file.path(baseWD, dataWD, current_fy,
                                                       paste0("health table fy ", current_fy, ".csv") ), row.names=FALSE)
  warning(paste("A new file was created:", file.path(baseWD, dataWD, current_fy,
                                                     paste0("health table fy ", current_fy, ".csv"))))
}
health_table <- read.dtable(file.path(baseWD, dataWD, current_fy,  paste0("health table fy ", current_fy, ".csv")))
health_table[, month := as.character(as.yearmon(gsub(x=month, pattern="-", replace=" 20")))]
health_table <- setkey(health_table, month)[!J(as.character(as.yearmon(paste(current_month, calendar_year))))]
...
write.csv(health_table[order(team, as.yearmon(month))],
          file.path(baseWD, dataWD, current_fy,  paste0("health table fy ", current_fy, ".csv")), row.names=FALSE)
health_table <- read.dtable(file.path(baseWD, dataWD, current_fy,  paste0("health table fy ", current_fy, ".csv")))
health_table[is.na(con_errors), con_errors := 0]
# merge with all possibilities
health_table <- merge(all_combinations, health_table, all=TRUE, by=c("team", "month"))
# re-order - may not be necessary
health_table <- health_table[order(team, as.yearmon(month))]
# factor based on 12 consecutive months
health_table[, month := factor(month,
  levels = as.character(as.yearmon(fy_start)+0:11/12))]

p_health <- list()
for(i in seq_along(team_list)) {
  # create graph p_health
  p_health[[i]] = ggplot(data=health_table[team==team_list[i]],
    aes(x=month, fill=month, y=con_errors, ymax=1.3*con_errors, ymin=0))+
  ...
  labs(title = paste0(team_list[i], " Graph ", i,
    ...
    geom_text(vjust=0.5, hjust=-0.5,
              aes(x=month, fill=month, y=con_errors, label=con_errors),
...
if(!file.exists( file.path(baseWD, dataWD, current_fy,
  ...
  write.csv(wageTeam[order(team, month)], file.path(baseWD, dataWD, current_fy,
                                                    paste0("wage table fy ", current_fy, ".csv") ), row.names=FALSE)
  warning(paste("A new file was created:",
                file.path(baseWD, dataWD, current_fy,  paste0("wage table fy ", current_fy, ".csv"))))
}
wage_table <- read.dtable(file.path(baseWD, dataWD, current_fy,  paste0("wage table fy ", current_fy, ".csv")))
wage_table[, month := as.character(as.yearmon(gsub(x=month, pattern="-", replace=" 20")))]
wage_table <- setkey(wage_table, month)[!J(as.character(as.yearmon(paste(current_month, calendar_year))))]
...
write.csv(wage_table[order(team, as.yearmon(month))],
          file.path(baseWD, dataWD, current_fy,  paste0("wage table fy ", current_fy, ".csv")), row.names=FALSE)
wage_table <- read.dtable(file.path(baseWD, dataWD, current_fy,  paste0("wage table fy ", current_fy, ".csv")))
# merge with all possibilities
wage_table <- merge(all_combinations, wage_table, all=TRUE, by=c("team", "month"))
# re-order - may not be necessary
wage_table <- wage_table[order(team, as.yearmon(month))]
# factor based on 12 consecutive months
wage_table[, month := factor(month, levels = as.character(as.yearmon(fy_start)+0:11/12))]

p_wage <- list()
for(i in seq_along(team_list)) {
  # create graph p_health
  p_wage[[i]] = ggplot(data=wage_table[team==team_list[i]], aes(x=month, fill=month, y=con_errors, ymax=max(round(con_errors)+1, 1.25*con_errors)))+
    geom_bar(width=0.5, stat="identity", color="black", size=1, fill="#C282D6", position=position_dodge(0.5))+
    labs(title = paste0(team_list[i], " Graph ", i, ".5.1 - # Consumers w/ Minimum Wage Errors\n(Report 2205 ran ",
                        gsub(x=run_date, pattern="_", replace="-"), ")"),
         y="# consumers with\n minimum wage errors")+
    geom_text(vjust=0.5, hjust=-0.5, aes(x=month, fill=month, y=con_errors,
                                         label=con_errors),
              angle=90, position=position_dodge(0.5), size=3, na.rm=TRUE)+
    my_theme+
    theme(legend.position="top",
    ...
    scale_y_discrete(breaks = my_breaks(n= wage_table[team==team_list[i]][, max(con_errors, na.rm=TRUE)] ))
}

### self sufficiency matrix ###

ssm_summary[, supervisor :=
  factor(supervisor, levels=ssm_summary[order(at_least2_ssms), supervisor])]
p_ssm <- ggplot(data = ssm_summary,
  aes(x=supervisor, y=at_least2_ssms, ymax=120, ymin=0))+
  geom_bar(stat="identity", color="black", fill="#C282D6", width=0.5)+
  # labs(title="Percent Case Load with at least 1 SSM in Last Year\n Note: Case Load is in Parentheses")+
  ggtitle(expression(atop("Percent Case Load with at least 2 SSM since 3/1/14",
    atop(italic("Note: Case Load is in Parentheses"), "")))) +
  coord_flip()+my_theme+
  theme(axis.ticks.x = element_blank(), axis.text.x = element_blank() )+
  geom_text(vjust=0.5, hjust=-0.1, aes(x=supervisor, y=at_least2_ssms, label=paste0(round(at_least2_ssms), "% (", case_load, ")" )),
            angle=0, size=3, na.rm=TRUE)

#### save results ####
if( !file.exists(file.path(baseWD, resultsWD, current_fy, current_month))) {
  dir.create(file.path(baseWD, resultsWD, current_fy, current_month), recursive=TRUE)
}
